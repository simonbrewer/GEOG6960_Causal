...
The goodness-of-fit value indicates that this model fits well (i.e. $p>0.05$), but it is close to the threshold. We can explore which of the missing paths may be the most useful to include to improve the fit by calculating the modification indices. 
...
Now try to fit the full model as described by the DAG below. Build the formula, then fit the model and check the Chis-squared test. Does this give a good fit? Are there any missing paths that should be included? Try visualizing the model to see the results.

![lavaanPlot](images/keeley_sem5.png) 
